...
public class SendPostsActivity extends Activity {
	...
	private View.OnClickListener btClick = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (bt_takePictrue == v) {
				if (takePhotoTimes >=3) {
					ShowToast.ShowTos(SendPostsActivity.this, R.string.more4);
				} else {
					if(isIntentAvailable(SendPostsActivity.this, MediaStore.ACTION_IMAGE_CAPTURE)){
						takePicture();
					}
				...
				if(CheckConnectNet.isNetworkConnected(SendPostsActivity.this)){
					if (takePhotoTimes >= 0) {
						   //提交文字
					...
					Toast.makeText(SendPostsActivity.this, "网络连接失败", 3000).show();
				}
	...
	private void takePicture() {
			...
			if (android.os.Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)){
				...
				ContextWrapper cw = new ContextWrapper(SendPostsActivity.this);
	...
	private  void  sendPostMsg(){
		...
		if (title.equals(null) || content.equals(null)) {
			ShowToast.ShowTos(SendPostsActivity.this,R.string.pleaseenter);
		} else {
			String html= madeHtml(strUploadPicture, content);
			
			List<NameValuePair> params_Settheme = new ArrayList<NameValuePair>();
			params_Settheme.add(new BasicNameValuePair("code",ShareData.REQUEST_CODE));
			...
			new SetTheme(SendPostsActivity.this,ShareData.LANSHAOQI_ADDRESS_DOPOST,params_Settheme, ConnectNetAsyncTask.SETTHEME).execute();
		}
